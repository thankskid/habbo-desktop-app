const j = global.jQuery || (global.jQuery = require('jquery-easing')),
  { remote: e, ipcRenderer: t } = require('electron');
...
function switchTab(e, t) {
  if (e === s) return;
  null !== s && (n[s].style.visibility = 'hidden'),
    (n[e].style.visibility = 'visible'),
    (n[e].style.position = 'absolute'),
    (n[e].style.width = '100%'),
    (n[e].style.bottom = '0');
  const i = document.getElementsByClassName('me__tab');
  for (let n = 0; n < i.length; n++)
    i[n].getAttribute('tab') === e &&
      (i[n].classList.add('me__tab--active'), t && changeHeaderStyle(e, i)),
      i[n].getAttribute('tab') === s &&
        i[n].classList.remove('me__tab--active');
  s = e;
}
...
function changeHeaderStyle(e) {
  ...
  'hotel' === e &&
    (t.classList.add('me__header--app'),
    (s.style.cssFloat = 'none'),
    (s.style.borderTopLeftRadius = '0px'),
    (s.style.borderBottomLeftRadius = '0px'),
    (n[e].style.top = '0px'),
    a.classList.add('notransition'),
    a.classList.add('me__tab--app'),
    a.offsetHeight,
    a.classList.remove('notransition'),
    (l.style.display = 'none'),
    (i.style.fontSize = '20px'),
    (i.style.transform = 'rotate(45deg)'),
    i.classList.remove('fa-expand-arrows-alt'),
    i.classList.add('fa-arrows-alt-v')),
    'index' === e &&
      (t.classList.remove('me__header--app'),
      (s.style.cssFloat = 'right'),
      (s.style.borderTopLeftRadius = '5px'),
      (s.style.borderBottomLeftRadius = '5px'),
      (n[e].style.top = '55px'),
      a.classList.add('notransition'),
      a.classList.remove('me__tab--app'),
      a.offsetHeight,
      a.classList.remove('notransition'),
      (l.style.display = 'inline-block'),
      (i.style.fontSize = 'inherit'),
      (i.style.transform = 'rotate(0)'),
      i.classList.add('fa-expand-arrows-alt'),
      i.classList.remove('fa-arrows-alt-v'));
}
  ...
  e && n[s].reload(),
    t.once('logged-in', (e, s) => {
      s &&
        ...
        'index' === s && n[s].reload(),
        'hotel' === s && (n.index.reload(), n.hotel.reload()));
    });
}
  ...
  return new Promise((l, o) => {
    if (e in n) l();
      ...
      (r.src = t),
        ...
        r.setAttribute('plugins', ''),
        r.setAttribute('id', e),
        r.setAttribute('class', 'me__webview'),
        null !== a && r.setAttribute('preload', a),
        o.appendChild(r),
        (n[e] = r),
        r.addEventListener('will-navigate', (e) => {
          switch (e.url) {
            case url + '/logout':
              logout();
              break;
      ...
      (u.src = `../resources/img/${i}`),
        (u.style.verticalAlign = '-5.5px'),
      ...
      (m.innerHTML = s),
        c.appendChild(u),
        c.appendChild(m),
        c.setAttribute('tab', e),
        c.setAttribute('id', e + '-btn'),
        c.classList.add('me__tab'),
        d.appendChild(c),
...
async function init() {
  await createTab('index', url + 'app/start', 'Home', 'home.gif'),
    setTimeout(() => switchTab('index', !1), 500);
}
init().then(() => console.debug('Launcher home initialized'));
